...
const fetchWeather = (provider, apiKey, feelsLike) => {
  return new Promise((resolve, reject) => {
    ...
    const manualLocation = settingsStorage.getItem('manualLocation');
    const lat = settingsStorage.getItem('lat');
    const lon = settingsStorage.getItem('lon');

    if (manualLocation === 'true' && lat && lon) {
      fetchFunc(apiKey, feelsLike, lat, lon)
        .then(resolve)
        .catch(reject);
